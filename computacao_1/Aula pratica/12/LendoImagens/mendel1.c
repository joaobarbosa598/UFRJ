...
int main()
{
    double x,xx,y,cx,cy;
    int iteration,hx,hy;
    int itermax = 10000;      /* how many iterations to do    */
    double magnify=10.0;     /* no magnification     */
    ...
    for (hy=1;hy<=hyres;hy++)  {
        for (hx=1;hx<=hxres;hx++)  {
            ...
            if (iteration != itermax)  {
                if (iteration > itermax / 2000) 
                    color((50 * iteration) % 255,0,0);
                else
                    color(0, (50 * iteration) % 255,0);
            }
            else color(0,0,180);
            /* printf("iteration %d\n", iteration); */
        }
